...
- NextJS components (routes, components, layouts etc) are server-side components by default
- Components that uses effects, events etc must be declared as client-side components by declaring the `'use client'` in the JS/TS file.