...
By using the Burp's interceptor on the target website, we found a _stockApi_ payload in the POST request after clicking the "Check stock" button:
...
By using the Burp's interceptor on the target website, we found a `productId=1&storeId=1` payload in the POST request after clicking the "Check stock" button.
