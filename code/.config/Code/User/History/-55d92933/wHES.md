...
Sometimes the the `/robots.txt` endpoint is not accessible but some hidden endpoints are written in clear in the source code, so by just inspecting it we can find hidden pages.
