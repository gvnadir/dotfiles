...
### Basic SSRF against another BE system
...
Now we access to the admin interface on `stockApi=http://192.168.0.1:8080/admin` where we can delete the user _carlos_
...
The error we receive is the following: _Sorry, file type application/x-php is not allowed..._

In this case, the workaround was to just change the `Content-Type` in the request, from `image/jpeg` to `application/x-php`

This means that there is no control about the file it's actually sent, but the only control was the `Content-Type` value in the header.
